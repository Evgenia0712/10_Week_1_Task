...
console.log(printMessage('Привет, меня зовут Женя'));
...
console.log(thirdFunction(5)); //Выведет 12
...
guestName = 'Настя';
function GreetinguestNameg(guestName = 'Гость') {
	return `Привет, ${guestName}!`;
}

console.log(GreetinguestNameg(guestName));
...
const total = function (k, l) {
	return k * l;
};

console.log(total(3, 5)); //Выведет 15
...
function calculateYearsSinceLastOlympics(a, lastOlympicsYear) {
	return a - lastOlympicsYear;
}
...
const friendYearOfBirth = 1992;

function calculateAge(year) {
	return year - friendYearOfBirth;
}

console.log(calculateAge(2023));
...
function yourOld(yearBirth) {
	return now - yearBirth;
}

console.log(yourOld(1992));
...
let results = function (z, r) {
	return z ** r;
};


console.log("Результат: " + results(2, 2));